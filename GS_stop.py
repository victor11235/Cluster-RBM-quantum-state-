...
class RBM:
    ...
    def __call__(self, output):
        ...
        gs.run(n_iter=5000, out=output, show_progress = False, callback = [nk.callbacks.EarlyStopping(monitor='mean', patience=80,                   min_delta=0.00001), nk.callbacks.Timeout(600)])
        ...
        return finalEng, state, runTime, n_iters
...
def runDescentCS(N,B,Ak,alpha,j):
    ...
    # Run RBM
    eng, state, runTime, n_iters = rbm("2021_summer_data/stop_log/con_Logs"+str(N)+str(j)) #where _call_ will be invoked
    return eng, state, runTime, n_iters
...
for i in range(11):  #here put N-1 (maximum)
    ...
    for i in range(len(hisIt)):
        ...
        engTemp, stateTemp, runTimeTemp, n_itersTemp = resultsSR[i]
    ...
    data = [engErr, stateErr, runTime, n_iters]
    fileName = "2021_summer_data/stop_0.01_0.00001_80ep_10min/stop_con_N"+str(N)+"M" + str(M)+".json"

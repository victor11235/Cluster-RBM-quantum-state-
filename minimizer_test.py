...
def runDescentCS_ex(N,B,Ak,alpha,state_list, Filling, i, j):
    ...
    # Run RBM
    eng, state = rbm("2021_summer_data/excited_hist/ex_data_var_rmsprop_3rd_excited_Logs"+str(N) + str(i)+'_' + str(j)) #where _call_ will be invoked
    return eng, state
...
def combined(N,B,Ak,alpha,j):
    ...
    eng = np.loadtxt("2021_summer_data/0.15%_error_4th_excited_eng.json")
    state = np.loadtxt("2021_summer_data/0.15%_error_4th_excited_state.json", dtype = complex)
    eng = eng.tolist()
    state = state.tolist()
    
    state[0] = np.array(state[0])
    state[1] = np.array(state[1])
    state[3] = np.array(state[2])
    
    v_0, v_1, v_2 = state[0], state[1], state[2]
    e_0, e_1, e_2 = eng[0], eng[1], eng[2]
...
for i in range(len(hisIt)):
    ...
    er_state_temp = err_sta(state_temp, v[3], N)
    er_eng_temp = np.abs(eng_temp - e[3])
...
fileName = "2021_summer_data/ex_hist_var_rmsprop_3rd_excited_N"+str(N)+"M" + str(M)+".json"
...
fileName_state = "2021_summer_data/State_list/var_rmsprop_3rd_excited_states.json"

...
def runDescentCS_ex(N,B,Ak,alpha,state_list, Filling, i, j):
    ...
    # Run RBM
    eng, state = rbm("2021_summer_data/excited_hist/ex_data_var_rmsprop_only_excited_Logs"+str(N) + str(i)+'_' + str(j)) #where _call_ will be invoked
    return eng, state
...
def combined(N,B,Ak,alpha,j):
    ...
    eng = np.loadtxt("2021_summer_data/0.1%_error_excited_eng.json")
    state = np.loadtxt("2021_summer_data/0.1%_error_excited_state.json", dtype = complex)
    eng = eng.tolist()
    state = state.tolist()
...
fileName = "2021_summer_data/ex_hist_var_rmsprop_only_excited_N"+str(N)+"M" + str(M)+".json"
...
for i in range(len(hisIt)):
    fileName_state = "2021_summer_data/State_list/var_rmsprop_only_excited_states"+str(i)+".json"

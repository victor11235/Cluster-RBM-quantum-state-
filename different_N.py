...
class RBM:
    def __init__(self, N, hamiltonian, hilbertSpace, machine):
        ...
        # Define optimizer
        self.optimizer = optax.rmsprop(learning_rate = 0.005, decay = 0.95, eps = 10**-2, initial_scale = 0.02)
        ...
        # Variational state
        self.vs = nk.variational.MCState(self.sampler, self.machine, n_samples=15000, n_discard=1500) #discarded number of samples 
    ...
    def __call__(self, output):
        ...
        gs.run(n_iter=20000, out=output, show_progress = False)
...
def runDescentCS_ex(N,B,Ak,alpha,state_list, Filling, i, j):
    ...
    # Run RBM
    eng, state = rbm("2021_summer_data/excited_hist/ex_data_var_post_0_excited_Logs"+str(N) + str(i)+'_' + str(j)) #where _call_ will be invoked
    return eng, state
...
for i in range(len(hisIt)):
    ...
    er_state_temp = err_sta(state_temp, v[0], N)
    er_eng_temp = eng_temp - e[0]
...
fileName = "2021_summer_data/ex_hist_var_post_0_excited_N"+str(N)+"M" + str(M)+".json"
...
fileName_state = "2021_summer_data/State_list/var_post_0_excited_states_N10.json"
